...
class UserContext extends PHPUnit_Framework_TestCase implements SnippetAcceptingContext {
    /**
     * @var InMemoryStorage
     */
    private $storage;
    ...
    public function iRegisterWithNameAndSurnameAndEmailAndPassword($name, $surname, $email, $password)
    {
        try {
            ...
            $name = new \Workly\Domain\Model\User\Name($name, $surname);
            $email = new Email($email);
            $password = new Password($password);
            $hasher = new Hasher(new \Illuminate\Hashing\BcryptHasher);
